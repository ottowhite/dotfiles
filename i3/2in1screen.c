...
void rotate_screen(){
	sprintf(command, "xrandr --output eDP-1-1 --rotate %s", ROT[current_state]);
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "ELAN2514:00 04F3:2A55", COOR[current_state]);
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "ELAN2514:00 04F3:2A55 Stylus Pen (0)", COOR[current_state]);
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "ELAN2514:00 04F3:2A55 Stylus Eraser (0)", COOR[current_state]);
	system(command);
}
